  ...
  return selectedUser ?  (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <img src={assets.videoCall} alt='videoCall' className='w-10 h-10 ' />
        <img src={assets.audioCall} alt='audioCall' className='w-10 h-10 ' />
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messages.map((msg,index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !==  authUser._id && 'flex-row-reverse'}`}>
            {msg.image ? (
              <img src={msg.image} className='max-w-[200px] border border-gray-700 ruonded-lg overflow-hidden mb-8' />
            ):(
              <>
        ...
        <div className='flex flex-1 items-center bg-gray-100/12 rounded-full'>
          {isListening ? (
            <div>
              ...
              <img src={assets.dustbin} className='w-10 h-10 cursor-pointer' onClick={()=>setIsListening(false)} alt='dustbin' />
            </div>
          ):(
            <>
