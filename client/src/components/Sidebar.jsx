  ...
  return (
    <div className={`bg-[#818582]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
        <div className='pb-5'>
            ...
            <div className='bg-[#282142] rounded-full items-center gap-2 flex py-3 px-4 mt-5'>
        ...
        <div className='flex flex-col'>
            {filteredUsers.map((user,index)=>(
                <div key={index} onClick={()=>{setSelectedUser(user);setUnseenMessages(prev=>({...prev,[user._id]:0}))}} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                    ...
                    {
                        unseenMessages[user._id] > 0 && <p className='absolute top-4 right-4 text-xs w-5 flex justify-center items-center
